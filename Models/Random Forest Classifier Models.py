...
clf = ensemble.RandomForestClassifier(n_estimators=500,verbose=1,n_jobs=-1)
...
testdf[['id', 'cuisine']].to_csv("rfc1.csv")
# Public Score (rfc1): 0.75472
...
# 2. Random Forest Classifier (Model 2):
# Using test.json file
...
submission_df.to_csv('rfc2.csv', index=False)

# Public Score (rfdc2): 0.75372
...
new_test_df[['id', 'cuisine']].to_csv("rfc3.csv")
#Accuracy (rfc3): 0.7541899441340782


# In[4]:


# 4. Random Forest Classifier (Model 4):
# Using random 30% training data (from train.json) for testing 
# and the remaining 70% training data (from train.json) for training
...
new_test_df[['id', 'cuisine']].to_csv("rfc4.csv")

print("done")
#Accuracy (rfc4): 0.7318435754189944
